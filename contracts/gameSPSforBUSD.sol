...
contract gameSPSforBUSD is Ownable, Pausable, VRFConsumerBaseV2 {
    uint128 public bank;
    ...
    IERC20 constant BUSD = IERC20 (address(0x277bf3B4aeA25936855b511efaAf3B2Db4c8DBDC));// FSPS token
    ...
    //↓↓↓↓↓↓// PLAY WITH SC //↓↓↓↓↓↓
    ////////////////////////////////
    function startGameSPS (uint _amount, uint128 _playerChoice) external whenNotPaused() {
        require(_amount>0.001 ether && _amount<=bank, "Wrong value");
        require(_playerChoice<=MAX_CHOICE_NUMBER, "Wrong choice number");
        (bool success, bytes memory response) = address(BUSD).call(
    ...
    function fulfillRandomWords(uint256 s_requestId, uint256[] memory randomWords) internal override {
        ...
        || SPS[s_requestId].playerChoice==2  && SPS[s_requestId].casinoChoice==0
            ...
            abi.encodeWithSignature(
                ...
                SPS[s_requestId].winner,
                SPS[s_requestId].bidValue*2-ownerAmount)
            );
    ...
    /////////////////////////////////////////
    function startPVPGameSPS (uint _amount, bytes32 _hashedChoice) external whenNotPaused() {
        require(_amount>0.001 ether, "min 0.001BNB");
        (bool success, bytes memory response) = address(BUSD).call(
            abi.encodeWithSignature(
                "transferFrom(address,address,uint256)",
                _msgSender(),
        ...
        spsPVP[gameNumber].bidValue=_amount/2;
    ...
    function makeChoicePVP (uint _amount, uint _gameNumber, uint128 _choice) external {
        require(!spsPVP[_gameNumber].withrawPriseOrStopped, "Prise received or stopped");
        require(_choice<=MAX_CHOICE_NUMBER, "Wrong choice number");
        require(block.timestamp<spsPVP[_gameNumber].endsAt, "Game over");
        require(_amount==spsPVP[_gameNumber].bidValue, "Wrong bid value");
    ...
    function revealChoice (uint _gameNumber, uint128 _choice, string calldata _secret) external {
        ...
        if (spsPVP[_gameNumber].firstChoice==spsPVP[_gameNumber].secondChoice) {
            ...
            require(success && abi.decode(response, (bool)), "Failed to send funds!");
            (bool success_, bytes memory response_) = address(BUSD).call(
            abi.encodeWithSignature(
                "transfer(address,uint256)",
                spsPVP[gameNumber].second,
                spsPVP[_gameNumber].bidValue)
            );
            require(success_ && abi.decode(response_, (bool)), "Failed to send funds!");
            emit gameEnd(_gameNumber, spsPVP[_gameNumber].winner);
        }
        ...
        || spsPVP[_gameNumber].firstChoice==2  && spsPVP[_gameNumber].secondChoice==0
            ...
            abi.encodeWithSignature(
                "transfer(address,uint256)",
                spsPVP[_gameNumber].winner,
                spsPVP[_gameNumber].bidValue*3-ownerAmount)
            );
            require(success && abi.decode(response, (bool)), "Failed to send funds!");
            emit gameEnd(_gameNumber, spsPVP[_gameNumber].winner);
        }
            ...
            abi.encodeWithSignature(
                "transfer(address,uint256)",
                spsPVP[_gameNumber].winner,
                spsPVP[_gameNumber].bidValue*2-ownerAmount/2)
            );
            require(success && abi.decode(response, (bool)), "Failed to send funds!");
            (bool success_, bytes memory response_) = address(BUSD).call(
            abi.encodeWithSignature(
                "transfer(address,uint256)",
                spsPVP[_gameNumber].first,
                spsPVP[_gameNumber].bidValue-ownerAmount/2)
            );
            require(success_ && abi.decode(response_, (bool)), "Failed to send funds!");
            emit gameEnd(_gameNumber, spsPVP[_gameNumber].winner);
        }
    ...
    function addBankBUSD(uint _amount) external {
        (bool success, bytes memory response) = address(BUSD).call(
            abi.encodeWithSignature(
                "transferFrom(address,address,uint256)",
                _msgSender(),
